...
LINQ，全称是**Language Integrated Query (语言集成查询)**，允许在c#中使用一种直观且一致的语法来查询不同的数据源，比如集合，sql数据库，xml文档等

如何使用？首先命名空间：System.Linq

LINQ提供两种主要的语法形式：查询语法和方法语法

五大常用关键字：

1. from 数据源
2. where 设定条件，过滤数据
3. select 选择特定字段或者形成新的数据结构
4. orderby 对结果进行排序
5. group 分组数据
...
## 5. c#中，public，private，protected修饰符分别有什么作用？
...
C# 中，类型分为**值类型**和**引用类型**，核心区别是**存储位置不同**：值类型默认存储在**栈（Stack）** 上（轻便、自动释放），引用类型存储在**堆（Heap）** 上（需 GC 回收），且引用类型会在栈上保存一个 “指向堆内存的地址”

值类型：int，float，char，enum枚举，struct结构体等等，复制时copy完整值
引用类型：容器 (List，Dictionary等等)，class，delegate委托，string等等，复制时copy地址，共享堆数据

**特殊场景：值类型的 “装箱” 与 “拆箱”**

值类型和引用类型可通过 “装箱” 和 “拆箱” 转换，但会产生额外开销（堆内存分配和 GC 垃圾）：

- **装箱**：值类型 → 引用类型（如 `int` → `object`），在堆上创建临时对象，产生 GC 垃圾。
- **拆箱**：引用类型 → 值类型（如 `object` → `int`），需先确保引用指向的值类型，否则抛异常。
